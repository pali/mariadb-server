...
inline void recv_sys_t::trim(const page_id_t page_id, lsn_t lsn)
{
	...
	for (recv_sys_t::map::iterator p = pages->lower_bound(page_id);
		...
		recv_sys_t::recs_t::list& log = p->second.log;

		for (auto prev = log.before_begin(), end = log.end();;) {
			auto i = prev;
			if (++i == end) {
				break;
			}
			...
			DBUG_LOG("ib_log",
				 "Discarding " << get_mlog_string(i->type)
				 << " for " << p->first << " at "
				 << i->start_lsn);
			log.erase_after(prev);
		}
			    ...
			    lsn_t end_lsn)
{
	...
	static const recs_t empty_recs{recs_t::RECV_NOT_PROCESSED, {}};

	std::pair<map::iterator, bool> p = pages->insert(
		map::value_type(page_id, empty_recs));
	recv_sys_t::recs_t& recs = p.first->second;
	recs_t::list::iterator i = recs.log.before_begin(),
		end = recs.log.end();
	...
	default:
		if (!p.second) {
			for (auto next = i; ++next != end; i = next);
		}
	}

	i = recs.log.emplace_after(i, recv_t{type, uint32_t(rec_end - body),
				       NULL, lsn, end_lsn});

	recv_data_t** prev_field = &i->data;
			      ...
			      mlog_init_t::init* init = NULL)
{
	...
	for (const recv_t& recv : p->second.log) {
...
void recv_recover_page(buf_page_t* bpage)
{
	...
	mutex_enter(&recv_sys.mutex);
	if (recv_sys.apply_log_recs && recv_sys.pages) { // FIXME: need both?
...
void recv_apply_hashed_log_recs(bool last_batch)
{
	...
	if (!recv_sys.pages || recv_sys.pages->empty()) {
		goto done;
	}
